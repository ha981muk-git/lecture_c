...
#include <errno.h>	/* wegen der Fehlernummer bei malloc() */
...
struct node * newElement(int aNum, char * aNam, double aPreis) {
	...
	/* passenden Speicher besorgen */
	if ((p_help = (struct node *)malloc(100000000*sizeof(struct node))) == NIL) {
	
		/* Fehlerbehandlung nachdem malloc() schief ging */
		
		printf("Fehler bei der Speicherallokierung fuer Artikel Nummer %i\n", aNum);
		fprintf(stderr, "newElement(): malloc() error %i (%s) on aNum %i\n",
						errno, strerror(errno), aNum);
						
		/* !!! Achtung, hier noch die bisherige Kette loeschen */
		/*  ==> Hausaufgabe: p_first mit uebergeben und deleteList() aufrufen */
		
		exit (errno);		/* harter Abbruch mit Export der Fehlernummer */
	}
...
void deleteList(struct node * p) {	/* ganze Liste rekursiv loeschen */
	...
	printf("loesche Artikel: Nummer: %i, Bezeichnung: %s, Preis: %6.2lf\n",
			p->art.artNum, p->art.name, p->art.preis);
	free((void *)p);	/* wer malloc() sagt, muss auch free() sagen */
}
